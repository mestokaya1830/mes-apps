<template>
  <main>
    <div class="container">
      ...
      <h1 class="title">Mesfor Image Compression</h1>
      ...
      <form @submit.prevent="sendFile()" enctype="multipart/form-data" class="form">
        ...
        <ul class="progress-side">
          ...
          <div v-if="isLoder && !uploading" class="loader-con">
            <Loader />
          </div>
    ...
    async sendFile(){
      ...
      try {
        if(files.length > 20){
          ...
          const res = await axios.post(`/api/compression/${this.quality}/${this.imagePath}` , formData, {
            onUploadProgress: e => {
