<template>
  <div class="editor-panel">
    ...
    <div class="editor-header-block">
      <div>
        <h1 class="title">{{ title }} {{ deliveries.length }}</h1>
      ...
      <router-link to="/deliveries/create" class="add-btn">
        ...
        <span>Neues Lieferung erstellen</span>
      </router-link>
    </div>
    ...
    <div class="customer-grid">
      <div v-for="item in deliveries" :key="item.id" class="customer-card">
        <!-- Card Header -->
...
export default {
  ...
  methods: {
    async getDeliveries() {
      try {
        const result = await window.api.getDocument('deliveries')
        if (!result.success) return
        this.deliveries = result.rows.map((item) => ({
          ...item,
    ...
    async deleteDelivery(id) {
      if (confirm('Sind Sie sicher, dass Sie dieses Lieferung löschen möchten?')) {
        try {
          const result = await window.api.deleteDocument('deliveries', id)
          if (result.success) this.getDeliveries()
        } catch (error) {
          console.error(error)
        }
      }
    ...
    searchFilter() {
      if (this.search_box && this.search_box.trim() !== '') {
        this.deliveries = this.search.filter(
          (item) =>
            ...
            this.formatDeliveryId(item.id).toLowerCase().includes(this.search_box.toLowerCase())
        )
      } else {
        this.deliveries = this.search
      }
    },
    dateFilter() {
      if (this.date_box_start && this.date_box_end) {
        ...
        this.deliveries = this.search.filter((item) => item.date == this.date_box_start)
      } else {
        this.deliveries = this.search
      }
    },
