<template>
  <div>
    <div v-if="deliveries" class="preview-panel" >
      <div class="printable">
        <!-- Header -->
        ...
        <div class="recipient">
          <div class="recipient-address">
            ...
            <div class="company-name-subtitle">
              {{ deliveries.customer.company_name }}
            </div>
            <div class="meta-label">{{ deliveries.customer.address }}</div>
            <div class="meta-label">
              {{ deliveries.customer.postal_code }}
              {{ deliveries.customer.city }}<br />
              {{ deliveries.customer.country }}
            </div>
          </div>
          <!-- delivery Details -->
          <div v-if="deliveries" class="recipient-details">
            ...
            <div class="meta-row">
              ...
              <span class="meta-value">{{
                formatCustomerId(deliveries.customer.id)
              }}</span>
            </div>
            <div class="meta-row">
            ...
            <div class="meta-row">
              ...
              <span class="meta-value">{{ formatDate(deliveries.date) }}</span>
            </div>
            <div class="meta-row">
              <span class="meta-label">Lieferreferenz:</span>
              <span class="meta-value">{{ deliveries.delivery_reference }}</span>
            </div>
          </div>
        </div>

        <!-- Positions -->
        <div class="positions-section" v-if="deliveries.positions.length">
          <table class="positions-table">
            ...
            <tbody>
              <tr v-for="(pos, index) in deliveries.positions" :key="index">
        ...
        <div class="status-section">
          <p><strong>Status:</strong> {{ getStatusText(deliveries.delivery_status) }}</p>
          <p v-if="deliveries.delivered_by">
            <strong>Ausgeliefert von:</strong> {{ deliveries.delivered_by }}
          </p>
          <p v-if="deliveries.received_by">
            <strong>Empfangen von:</strong> {{ deliveries.received_by }}
          </p>
        </div>

        <div v-if="deliveries.note" class="notes-section">
          <h3>Bemerkungen:</h3>
          <p>{{ deliveries.note }}</p>
        </div>
        <!-- Contact Person -->
        ...
        <div class="signatures-section">
          <div class="signature-box">
            ...
            <p class="signature-date">Datum: {{ formatDate(deliveries.delivery_date) }}</p>
          </div>
          <div class="signature-box">
            <div class="signature-line"></div>
...
export default {
  ...
  computed: {
    formatDeliveryId() {
      if (!this.deliveries || !this.deliveries.id) return ''
      const year = new Date().getFullYear()
      return `RE-${year}-${String(this.deliveries.id).padStart(5, '0')}`
    },
    formatOrderId() {
      if (!this.deliveries || !this.deliveries.id) return ''
      const year = new Date().getFullYear()
      return `AUF-${year}-${String(this.deliveries.order_id).padStart(5, '0')}`
    }
  },
  mounted() {
    this.getDeliveries()
  ...
  methods: {
    async getDeliveries() {
      ...
      try {
        const result = await window.api.getDocumentById(id, 'deliveries')
        this.deliveries = {
          ...result.rows,
